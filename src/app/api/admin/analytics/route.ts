...
export async function GET(req: NextRequest) {
  try {
    ...
    const [totalViews, uniqueSessionsRaw, topPages, topSearches, deviceBreakdown, locationBreakdown, topSearchesByLocation, dailyViewsRaw, adStats] = await Promise.all([
      ...
      // daily views for chart (last N days) — COUNT(*) returns a Postgres BigInt, converted below
      prisma.$queryRaw<{ date: Date; views: bigint }[]>`
    ...
    const dailyViews = dailyViewsRaw.map(r => ({ date: r.date, views: Number(r.views) }))
