...
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.fieldReport.create({
      data: {
        reporterName: typeof reporterName === 'string' ? reporterName.slice(0, 100) : null,
    ...
    const totalCount = await prisma.fieldReport.count({ where: { reporterPhone: reporterPhone.trim() } })

    return ok({ totalCount }, 201)
  } catch (e) {
